...
class loghandle:
    ...
    def error(self, error, LINE=None, SECOND=None, MINUTE=None, HOUR=None, AT=None):
        ...
        if AT is True:
            small_time.append(' - Second: '+str(date.second))
            small_time.append(' - Minute: '+str(date.minute))
            small_time.append(' - Hour: '+str(date.hour))
        if SECOND is True:
            small_time.append(' - Second: '+str(date.second))
        if MINUTE is True:
            small_time.append(' - Minute: '+str(date.minute))
        ...
        if len(self.setpath[0:]) >= 1:
            def write(error):
                ...
                if LINE is None:
                    if len(small_time) >= 1:
                        output =("\nLOGGED - "+str(refreshed_time[0])+" - "+str(small_time)+" --> "+string)
                    else:
                        output =("\nLOGGED - "+str(refreshed_time[0])+" - LINE:"+LINE+" --> "+string)
                else:
                    try:
                        if type(LINE) is str:
                            if len(small_time) >= 1:
                                output =("\nLOGGED - "+str(refreshed_time[0])+" - LINE:"+LINE+" - "+str(small_time)+" --> "+string)
                            else:
                                output =("\nLOGGED - "+str(refreshed_time[0])+" - LINE:"+LINE+" --> "+string)
                        else:
                            print("ERR C1 :: line_number MUST BE INT :: "+str(type(LINE))+"_NotAccepted")
                            return False
                    except Exception as e:
                        print(str(e))
                try:
                    log_write = open(
                        dir+"/logs/LOG "+str(datetime.datetime.now().date())+".txt", "a+")
